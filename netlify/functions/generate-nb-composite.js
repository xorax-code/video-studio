...
Hard rules: never add a second person or any human figure that was not already in Photo 1. Keep Photo 1's background, setting, and lighting. FRAMING (CRITICAL) — ALWAYS render a MEDIUM shot, regardless of Photo 1's framing. Even if Photo 1 is a tight close-up or shows two people close to the camera, PULL THE CAMERA BACK so every person is shown from at least the chest up with clear headroom and space around them, and NO single face is larger than about one-quarter (25%) of the frame height. A large face filling the frame trips Veo's person filter and the clip fails, so err on the side of too wide. Never crop tighter than this. EXCEPTION: if Photo 1 is a hand/arm-only product shot with no face, keep its exact crop unchanged.${hasProduct ? ' Replace the product held in the hand with the exact product from Photo 3 (same hand, grip, and scale).' : ''} Always remove any burned-in text, captions, or subtitles.`
