...
In BOTH cases: NEVER add a second person, a duplicate, or any extra human figure that was not already in Photo 1. BACKGROUND ART (critical): do NOT reproduce any detailed wall poster, anatomical or medical chart, skin/body diagram, infographic, or framed artwork from Photo 1 — even if Photo 1 clearly shows one. Replace it with a plain blank framed print of the SAME size and in the SAME position, and keep every other background object identical. Reproducing a real poster or chart causes a copyright/recitation block and the clip fails.${_analysisHint}\n\n`;
...
Hard rules: never add a second person or any human figure that was not already in Photo 1. Keep Photo 1's background, setting, and lighting. REPRODUCE THE BACKGROUND EXACTLY (critical): every object, prop, shelf item, bottle, and piece of décor must appear in the SAME place, count, shape, and color as Photo 1 — do NOT add, remove, move, resize, restyle, or invent any background object. Do NOT widen, zoom, or reveal any area outside Photo 1's frame EXCEPT the minimal pull-back permitted below for an oversized face. FRAMING (CRITICAL) — PRESERVE Photo 1's exact framing, camera angle, and the positions of BOTH people so the output matches the source video — do NOT recompose, zoom, re-center, or move anyone. The ONE exception: if a face would fill more than about 40% of the frame height, pull the camera back just enough to bring it under that (a very large photoreal face can trip Veo's person filter), keeping the same composition otherwise. Render it as a real photographic scene with the people in their original positions. TWO PEOPLE: replace ONLY the person the instruction targets by position (top/bottom/left/right) — NEVER default to the front, foreground, or lying-down person unless that IS the targeted one. Keep the OTHER person exactly as in Photo 1 (same face, identity, pose), just framed slightly back and smaller so you don't end up with two co-equal faces side-by-side filling the frame (the composition Veo blocks). EXCEPTION: if Photo 1 is a hand/arm-only product shot with no face, keep its exact crop unchanged.${hasProduct ? ' Replace the product held in the hand with the exact product from Photo 3 (same hand, grip, and scale).' : ''} Always remove any burned-in text, captions, or subtitles.`
  ...
  const parts = [];
  if (hasFrame) {
    parts.push({ text: 'Photo 1 — BASE SCENE / FRAME (reproduce its background, props, setting, and lighting EXACTLY — same objects in the same places — and any hand/arm/prop shown). Keep its framing and field of view; only if a person\'s face would fill more than about 40% of the frame height, pull the camera back the minimum needed to clear that (same background, no new props) — never crop tighter. Look at this photo to decide: does it contain a person, or only a hand/arm?:' });
  ...
  // Stable per-scene seed (passed from the client) so re-renders of the same frame
  // reproduce the SAME background instead of rerolling props each time. Best-effort per
  // the image API; the client bumps it on an intentional Redo to get a genuinely new take.
  const _seedVal = (body.seed != null && isFinite(Number(body.seed))) ? (Number(body.seed) >>> 0) : null;
  const requestObj = {
