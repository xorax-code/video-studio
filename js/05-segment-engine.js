  ...
  // --- Import a video straight from a pasted link (TikTok / IG / FB / YT) ---
  // Calls the fetch-remote-video Netlify function, which resolves + downloads the
  // clip server-side, stores it, and hands back a signed URL we pull into a File
  // and feed through the normal onVideoFileChange path.
  async function fetchRemoteVideo() {
    const input  = document.getElementById('dlLinkInput');
    const btn    = document.getElementById('dlFetchBtn');
    const status = document.getElementById('dlFetchStatus');
    const url = ((input && input.value) || '').trim();
    const setStatus = (msg, color) => { if (status) { status.style.display='block'; status.textContent=msg; status.style.color=color||'var(--text-3)'; } };
    if (!url) { setStatus('Paste a link first.', 'var(--danger)'); return; }
    if (window.location.protocol === 'file:') { setStatus('Link import only works on the live site.', 'var(--danger)'); return; }
    if (btn) btn.disabled = true;
    setStatus('Fetching video…');
    try {
      let headers = { 'Content-Type': 'application/json' };
      try { if (typeof _sb !== 'undefined' && _sb) { const s = await _sb.auth.getSession(); const tok = s && s.data && s.data.session && s.data.session.access_token; if (tok) headers['Authorization'] = 'Bearer ' + tok; } } catch(_) {}
      const resp = await fetch('/.netlify/functions/fetch-remote-video', { method:'POST', headers, body: JSON.stringify({ url }) });
      const info = await resp.json().catch(() => ({}));
      if (!resp.ok) { setStatus((info && info.error && info.error.message) || 'Import failed.', 'var(--danger)'); return; }
      setStatus('Downloading…');
      const vr = await fetch(info.url);
      if (!vr.ok) { setStatus('Could not download the imported video.', 'var(--danger)'); return; }
      const blob = await vr.blob();
      const file = new File([blob], info.filename || 'reference.mp4', { type: 'video/mp4' });
      onVideoFileChange({ target: { files: [file] } });
      setStatus('Imported \u2713' + (info.watermarkFree === false ? ' (has watermark)' : ''), 'var(--accent-2)');
      if (input) input.value = '';
    } catch (err) {
      setStatus('Import error: ' + ((err && err.message) || err), 'var(--danger)');
    } finally {
      if (btn) btn.disabled = false;
    }
  }
  window.fetchRemoteVideo = fetchRemoteVideo;
    ...
    if (hasExtras) {
      ...
      seg.veoExtras.forEach(function(extra, j) {
        ...
        if (extra.apiVideoUrl) {
          ...
          h += '<button onclick="fsClip(\'cardclipx-' + i + '-' + j + '\')" title="Fullscreen" style="padding:2px 7px;font-size:9px;font-weight:700;background:rgba(99,102,241,0.10);border:1px solid rgba(99,102,241,0.3);border-radius:5px;color:var(--accent-2);cursor:pointer;font-family:inherit;">&#x26F6;</button>';
          ...
          h += '</div></div>';
          h += '<video id="cardclipx-' + i + '-' + j + '" controls controlsList="nofullscreen" playsinline ondblclick="fsClip(\'cardclipx-' + i + '-' + j + '\')" preload="none" poster="' + ((seg && (seg.nbPreviewDataUrl || seg.frameDataUrl)) || '') + '" style="width:100%;border-radius:7px;max-height:180px;background:#000;display:block;object-fit:contain;" src="' + (extra.apiVideoRaw || extra.apiVideoUrl) + '"></video>';
        ...
        <div style="border-top:1px solid rgba(16,185,129,0.22);padding-top:8px;margin-top:4px;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
            ...
            <div style="display:flex;gap:5px;align-items:center;">
              <button onclick="fsClip('cardv-primary-${i}')" title="Fullscreen" style="padding:2px 8px;font-size:9px;font-weight:700;background:rgba(99,102,241,0.10);border:1px solid rgba(99,102,241,0.3);border-radius:5px;color:var(--accent-2);cursor:pointer;font-family:inherit;">⛶</button>
              <button id="regenFrameBtn-${i}" onclick="if(getGenerateMode&&getGenerateMode()==='api'){regenSceneFrame(${i});}else{showToast('Switch to Auto mode to regenerate via API.','info',4000);}" title="Regenerate the start frame for this scene (new pose/composite), then hit Regen to re-render the video" style="padding:2px 8px;font-size:9px;font-weight:700;background:rgba(167,139,250,0.10);border:1px solid rgba(167,139,250,0.35);border-radius:5px;color:#a78bfa;cursor:pointer;font-family:inherit;transition:all 0.15s;">↺ Frame</button>
              <button id="regenSceneBtn-${i}" onclick="if(getGenerateMode&&getGenerateMode()==='api'){regenSingleScene(${i});}else{showToast('Switch to Auto mode to regenerate via API.','info',4000);}" title="Regenerate this clip via API (re-renders the video from the current start frame)" style="padding:2px 8px;font-size:9px;font-weight:700;background:rgba(56,189,248,0.10);border:1px solid rgba(56,189,248,0.35);border-radius:5px;color:#38bdf8;cursor:pointer;font-family:inherit;transition:all 0.15s;">↺ Regen</button>
          ...
          <video id="cardv-primary-${i}" controls controlsList="nofullscreen" playsinline ondblclick="fsClip('cardv-primary-${i}')" preload="none" poster="${seg.nbPreviewDataUrl || seg.frameDataUrl || ''}" style="width:100%;border-radius:8px;max-height:200px;background:#000;display:block;object-fit:contain;" src="${seg.apiVideoRaw || seg.apiVideoUrl}"></video>
            ...
            <div style="background:rgba(99,102,241,0.06);border:1px solid rgba(99,102,241,0.22);border-radius:6px;padding:7px 8px;margin-bottom:5px;display:flex;flex-direction:column;gap:5px;">
              ...
              <div style="border-top:1px solid rgba(99,102,241,0.2);padding-top:6px;margin-top:2px;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                  ...
                  <div style="display:flex;gap:4px;align-items:center;">
                    <button onclick="fsClip('cardclipx-${i}-${j}')" title="Fullscreen" style="padding:2px 7px;font-size:9px;font-weight:700;background:rgba(99,102,241,0.10);border:1px solid rgba(99,102,241,0.3);border-radius:5px;color:var(--accent-2);cursor:pointer;font-family:inherit;">⛶</button>
                    <button id="regenExtraBtn-${i}-${j}" onclick="if(typeof getGenerateMode==='function'&&getGenerateMode()==='api'){regenExtraClip(${i},${j});}else{showToast('Switch to Auto mode to regenerate via API.','info',4000);}" title="Regenerate continuation clip" style="padding:2px 7px;font-size:9px;font-weight:700;background:rgba(56,189,248,0.10);border:1px solid rgba(56,189,248,0.35);border-radius:5px;color:#38bdf8;cursor:pointer;font-family:inherit;">↺ Regen</button>
                    <button onclick="(function(){var e=segments[${i}]&&segments[${i}].veoExtras&&segments[${i}].veoExtras[${j}];if(!e)return;e.apiVideoUrl=null;e.apiVideoMime=null;e.apiVideoRaw=null;debounceSave();renderSegments();})()" style="padding:2px 7px;font-size:9px;background:rgba(248,113,113,0.07);border:1px solid rgba(248,113,113,0.2);border-radius:5px;color:var(--danger);cursor:pointer;font-family:inherit;">✕</button>
                  </div>
                </div>
                <video id="cardclipx-${i}-${j}" controls controlsList="nofullscreen" playsinline ondblclick="fsClip('cardclipx-${i}-${j}')" preload="none" poster="${seg.nbPreviewDataUrl || seg.frameDataUrl || ''}" style="width:100%;border-radius:7px;max-height:180px;background:#000;display:block;object-fit:contain;" src="${extra.apiVideoRaw || extra.apiVideoUrl}"></video>
